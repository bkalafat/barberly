...
/// <summary>
/// Handles AppointmentCancelledEvent and creates email notifications in the outbox.
/// </summary>
public sealed class AppointmentCancelledEventHandler : INotificationHandler<AppointmentCancelledEvent>
{
    ...
    public async Task Handle(AppointmentCancelledEvent notification, CancellationToken cancellationToken)
    {
        ...
        var customerBody = await _templateService.RenderAppointmentCancellationAsync(
            appointmentDto, userDto, barberDto, serviceDto, shopDto, notification.CancelledAt);
        
        var customerNotification = NotificationOutbox.Create(
            "AppointmentCancelled",
            user.Email,
            user.FullName,
            "Randevunuz İptal Edildi - Barberly",
            customerBody,
            JsonSerializer.Serialize(new { appointmentDto.Id, notification.Start, notification.End, notification.CancelledAt }));

        await _outboxRepository.AddAsync(customerNotification, cancellationToken);

        // Create notification for barber
        var barberBody = await _templateService.RenderAppointmentCancellationAsync(
            appointmentDto, userDto, barberDto, serviceDto, shopDto, notification.CancelledAt);
